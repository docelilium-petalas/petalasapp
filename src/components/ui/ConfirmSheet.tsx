  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={() => responder(false)}
          className="absolute top-4 right-4 p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-neutral-800 transition-colors no-touch-target max-md:hidden"
          aria-label="Fechar"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <h3 className="text-base font-bold text-neutral-100 leading-snug tracking-tight">
          {pedido.titulo}
        </h3>

        {pedido.alvo && (
          <p className="mt-2.5 px-3 py-2 rounded-xl border border-border/40 bg-neutral-900/60 text-xs font-semibold text-neutral-200 truncate select-text">
            {pedido.alvo}
          </p>
        )}
